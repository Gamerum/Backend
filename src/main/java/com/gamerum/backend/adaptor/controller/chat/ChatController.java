...
public class ChatController {
    ...
    @GetMapping("/{chatId}")
    public ResponseEntity<ChatGetDTO> getChat(@PathVariable Long chatId) {
        return new ResponseEntity<>(ChatMapper.INSTANCE.chatToChatGetDTO(chatService.getByChatId(chatId)), HttpStatus.OK);
    }
    ...
    @Secured({"ROLE_USER", "ROLE_ADMIN"})
    @GetMapping
    public ResponseEntity<List<ChatGetDTO>> getChats(@RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Long profileId) {
        return new ResponseEntity<>(ChatMapper.INSTANCE.chatsToChatGetDTOs(chatService.getChats(page, profileId)), HttpStatus.OK);
    }
}